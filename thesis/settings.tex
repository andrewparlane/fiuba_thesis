...
% Create a new command for referencing figures with "Figura ?.?" as the link. Same for tables, sections and appendice
% Call with:
%  \figref{fig:figure-label}
%  \tabref{tab:table-label}
%  \secref{sec:section-label}{Section Name}
%  \appref{appendix:appendix-label}
...
\newcommand*{\appref}[1]{\hyperref[#1]{Apéndice~\ref{#1}}}
...
% listings settings for C code samples (appendix)
...
% Change the font size of block quotes (displaycquote)
