...
Este módulo calcula el CRC16 definido por el polinomio x16 + x12 + x5 + 1 y el valor inicial 0x6363, por una serie de bits. Esta implementación está diseñada para ser utilizado por tramas de recepción y de transmisión, por lo tanto no usa una interfaz. Las salidas y entradas son descritas en el \tabref{tab:ports_crc_a}.
...
Este módulo instancia el módulo \textbf{CRC\_A} y lo usa para verificar los CRCs de tramas recibidas y para calcular los CRCs de tramas a transmitir. Las entradas y salidas son descritas en el \tabref{tab:ports_crc_control}. Este módulo contiene un monitor por la \textit{rx\_interface} y uno por la \textit{tx\_interface}. En este caso el término “monitor” se refiere a que el mismo lee las señales de la interfaz para poder extraer los bits de las tramas. No se debe confundir con los monitores usados en los bancos de pruebas, aunque tienen una función parecida.
...
Las salidas y entradas son descritas en el \tabref{tab:ports_iso14443a}.
...
Este módulo sincroniza una señal al dominio del reloj para prevenir metaestabilidad~\cite{synch_arb_in_digital_systems}. Este módulo es parametrizado y sus parámetros se describen en el \tabref{tab:params_synchroniser}. Las salidas y entradas son descritas en el \tabref{tab:ports_synchroniser}.
...
La implementación es dos registros conectados como se muestra la \figref{fig:synchroniser}.
...
Debido a que el reloj puede detenerse durante las pausas, no está garantizado que haría un flanco ascendente del reloj mientras que la pausa está activa, por lo tanto es necesario usar un circuito asíncrono para retener las pausas hasta que el reloj comience de nuevo. El circuito usado es un FFD (Flip Flop D) con un reset y un set asíncronos, conectado como se muestra en la \figref{fig:pause_n_latch}. La entrada \textit{pause\_n\_async} es conectada a la reset asíncrona, así durante la pausa, la salida \textit{pause\_n\_latched} es ‘0’. La entrada D del FFD es un ‘1’ constante, así en el primer flanco ascendente del reloj después de la pausa, la salida \textit{pause\_n\_latched} vuelve a ‘1’. La entrada \textit{rst\_n} está conectada a la set asíncrona, así durante un reset la salida es ‘1’. Debido a que se usa un reset asíncrono el flanco descendente de la  \textit{pause\_n\_latched} es asíncrona al reloj, por lo tanto es necesario usar un sincronizador para prevenir metaestabilidad. La \figref{fig:pause_n_synch_and_latch} muestra el diseño final.

\begin{figure}[htb]
  \centering
  \includegraphics[width=1.0\textwidth]{./img/pause_latch_and_synch1_latching.drawio}
  \caption{Circuito asíncrono por la retención de pausas.}
...
Las salidas y entradas son descritas en el \tabref{tab:ports_pause_n_synch_and_latch}.
...
\begin{table}[htb]
  ...
  \begin{tabulary}{\linewidth}{l|l|L}
    ...
    clk&Entrada&Reloj. \\
    rst\_n&Entrada&Reset activo bajo. \\
    pause\_n\_async&Entrada&Pausa, asíncrona, desde el AFE. \\
...
La \figref{fig:pause_n_synch_and_latch_timing} muestra un diagrama de timing de este diseño cuándo el reloj está detenido durante la pausa entera.
